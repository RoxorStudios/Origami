...
<div class="section">
	<div class="box">
		<div class="padding">
			...
			<h1 class="boxtitle">{{ isset($single) ? $module->name : ($entry->id ? 'Edit entry' : 'New entry') }}</h1>
			@if($module->fields()->count())
				<form method="POST" action="{{ !$entry->uid ? origami_url('/entries/'.$module->uid.'/create') : origami_url('/entries/'.$module->uid.'/'.$entry->uid) }}" enctype='multipart/form-data'>
